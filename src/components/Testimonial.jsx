...
function GridFallback({ testimonials }) {
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {testimonials.map((t) => (
        <div key={t.id} className="bg-white rounded-2xl border border-sand p-5">
          <span className="text-orange text-2xl font-black leading-none block mb-2">"</span>
          <p className="text-forest/75 text-sm leading-relaxed mb-4">{t.text}</p>
          <div className="flex items-center gap-2 pt-3 border-t border-sand/60">
            ...
            <div>
              ...
              <p className="text-xs text-forest/40">{countryFlag(t.country)} {t.country}</p>
            </div>
          </div>
        </div>
      ))}
