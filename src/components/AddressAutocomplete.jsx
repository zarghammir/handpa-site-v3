...
export default function AddressAutocomplete({
  ...
  // `importLibrary("places")` resolves only once the places library is fully
  // loaded, and it's the loading path that exposes the new `AutocompleteSuggestion`
  // class (a plain `?libraries=places` script tag does not).
  useEffect(() => {
      ...
      .then((places) => {
        ...
        if (!places?.AutocompleteSuggestion) {
          console.warn(
            ...
            "ensure 'Places API (New)' is enabled on the Cloud project."
          );
          return;
        }
